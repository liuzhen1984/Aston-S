...
public final class CRC16 {
	...
	{
		crc = (short) (((crc >> 8)&0xff) ^ CRC16_TABLE[((crc & 0xff) ^ (data&0xff)) & 0xff]);
	    return crc;
	}
	...
	public static short updateBlock(byte[] data, int pos, int len, short crc) {
		for (int i = pos; i < pos+len; i++) {
			crc = (short) (((crc >> 8)&0xff) ^ CRC16_TABLE[((crc & 0xff) ^ (data[i]&0xff))&0xff]);
		}
		return crc;
	}
}
